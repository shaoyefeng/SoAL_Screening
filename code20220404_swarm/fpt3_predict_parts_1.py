...
import time
import numpy as np
from fpt_consts import MODEL_FOLDER, GPU_N
# from fpt_util import redirect_output
...
class PredictParts(object):
    ...
    def init_pred_model(self):
        ...
        self._model = predict_box.Model(MODEL_FOLDER)
    ...
    def append_new_task(self, pack):
        ...
        from fpt4_id_assign_1 import IdAssign  # NOTE: diff
    ...
    def predict_loop(self):
        ...
        while not self.stop:
            ...
            for i in range(l):
                ...
                if task_id is None:
                    print("[predict_parts%d] exit" % self._idx)
                    self.stop = True
                    break
                if task_id not in self._task_q_l:
                    ...
                    if img is None:
                        ...
                        # plot_ego("temp/img%06d%08d.png" % (pack[PACK_FRAME], i), pack[PACK_IMG], None)
                        # continue
                        img_l.append(img)
                        pack_l.append(pack)
            if not img_l:
            ...
            for i, img in enumerate(img_l):
                ...
                #     plot_ego("temp/img%06d%08d.png" % (pack[PACK_FRAME], i), pack[PACK_IMG], np.array(ypk[:, :, i]))  #TEST:PLOT_EGO
