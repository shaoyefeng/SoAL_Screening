...
from fpt0_util import load_dict
from fpt_consts import FLY_NUM, FLY_AVG_AREA, MODEL_SHAPE, MODEL_SHAPE_EXTEND, FEAT_SCALE_NORMAL, DURATION_LIMIT, \
    START_FRAME, FRAME_STEP, IMG_PROCESS_NUM, IMG_PROCESS_CAPACITY, FIX_VIDEO_SIZE
from fpt_bg import remove_bg2, norm_subbg, remove_bg3, calc_bg
...
class RegionSeg(object):
    def __init__(self, video, task_id, pred_q):
        ...
        self._total_frame = min(DURATION_LIMIT * int(self._meta["FPS"]+1), self._meta["total_frame"]) or self._meta["total_frame"]
        self._bg_filename = video.replace(".avi", ".bmp")

        self._gray_thresh = self._meta.get("GRAY_THRESHOLD", 150)
    ...
    def init_model_shape(self):
        if abs(self._feat_scale - FEAT_SCALE_NORMAL) > 2:
            ...
            self._model_shape = [int(MODEL_SHAPE[0] / s), int(MODEL_SHAPE[1] / s)] #[48,48]#
            self._model_shape_extend = (int(self._model_shape[0] * 1.5), int(self._model_shape[1] * 1.5))
        if FIX_SHAPE:
            print("!!!fix_shape=", FIX_SHAPE, " model_shape=", self._model_shape)
            self._model_shape = FIX_SHAPE
            self._model_shape_extend = (int(self._model_shape[0] * 1.5), int(self._model_shape[1] * 1.5))
        else:
            print("!!!auto_shape=", self._model_shape)

    def init_edge_mask(self):
        edge_mask = np.zeros([self._model_shape[0]-2, self._model_shape[1]-2])
        edge_mask = np.vstack([np.ones([1, self._model_shape[1]-2]), edge_mask, np.ones([1, self._model_shape[1]-2])])
        self._edge_mask = np.hstack([np.ones([self._model_shape[0], 1]), edge_mask, np.ones([self._model_shape[0], 1])])
    ...
    def proc_video(self):
        ...
        if FRAME_STEP:
            ...
            for seq in range(START_FRAME, self._total_frame):
                if FIX_VIDEO_SIZE:
                    self._cap.set(cv2.CAP_PROP_POS_FRAMES, seq)  # NOTE: bug
                ret, img = self._cap.read()
                if not ret:
                    print("read video error: " + self._video)
                    self._pred_q.put([self._task_id, None, seq, 0, None, 0])
    ...
    def init_bg(self):
        ...
        for i, roi in enumerate(self._roi):
            ...
            if CENTER_UNIFORM_RANGE > 0:
                ...
                img_bg_center_uniform = img_bg_f
    ...
    def proc_img2(self, img, frame):
        ...
        for i, roi in enumerate(self._roi):
            ...
            if region_n < FLY_NUM:  # need gmm
                ...
                # if i==13 and frame ==78401:
                #     print("[region_seg]#%d gmm_seg found nothing !!! (video %s, roi %d, frame %d)" % (self._task_id, self._video, i, frame))
                #     cv2.imwrite("temp/error_%d_%d.png" % (i, frame), img_no_bg)
                #     print(self._min_area, self._max_area)
                #     for c in contours:
            ...
            # cv2.imwrite("temp/img_no_bg-%d_%d_%d.png" % (self._task_id, i, frame), img_no_bg)  # test output bg
    ...
    def proc_one_img(self, img, roi, i, proc_center=True, no_center_img=False):
        ...
        if region_n < FLY_NUM:  # need gmm
            ...
            roi_regions = roi_regions[:FLY_NUM]
        if no_center_img:
            return img_no_bg, img_roi, [(reg[1], reg[2]) for reg in roi_regions]
        img_center_l = []
        pos_l = []
        for reg in roi_regions:
            if proc_center:
...
def center_img(img_gray, center, theta, out_shape, out_shape_w):
    # shift = -center[0] + in_shape[0] / 2, -center[1] + in_shape[1] / 2
    sub0 = int(center[1] - out_shape_w[0]/2)
    sub1 = int(center[0] - out_shape_w[1]/2)
    end0 = sub0 + out_shape_w[0]#img_gray.shape[0]+5
    end1 = sub1 + out_shape_w[1]#img_gray.shape[1]+5
    img_shift = img_gray[max(sub0, 0):end0, max(sub1, 0):end1]#ndimage.shift(img_gray, shift)
    if sub0 < 0:
        img_shift = np.vstack((np.full((-sub0, img_shift.shape[1]), 255), img_shift))
    if end0 > img_gray.shape[0]:
        img_shift = np.vstack((img_shift, np.full((end0 - img_gray.shape[0], img_shift.shape[1]), 255)))
    if sub1 < 0:
        # print(img_shift.shape, sub1)
        img_shift = np.hstack((np.full((img_shift.shape[0], -sub1), 255), img_shift))
    if end1 > img_gray.shape[1]:
        img_shift = np.hstack((img_shift, np.full((img_shift.shape[0], end1 - img_gray.shape[1]), 255)))
    img_rotate = ndimage.rotate(img_shift, theta+90)
    shape_r = img_rotate.shape
    sub0 = int((shape_r[0] - out_shape[0]) / 2)
    sub1 = int((shape_r[1] - out_shape[1]) / 2)
    return img_rotate[sub0:(sub0 + out_shape[0]), sub1:(sub1 + out_shape[1])]
...
if __name__ == '__main__':
    main()
# kernprof.exe -l .\fpt2_region_seg.py "D:\exp\video_todo\20190902_150503_A\20190902_150503_A.avi"
# python -m line_profiler .\fpt2_region_seg.py.lprof
