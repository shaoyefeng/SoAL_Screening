...
DATA_DIR = r"/media/syf/DataRecovery/LC_screening/SYF/LC25/center"
VIDEO_DIR = r"/media/syf/DataRecovery/LC_screening/SYF/LC25/video"
DRIVER_L = ["CSXTrp31 LC25XTrp31 LC25XShi31"]
...
SUMMARY_FIGURES = ["c_pos-female", "c_pos_h", "c_pos_t", "fc_speed", "fc_abs_av", "fc_acc", "fc_wing_m", "fc_theta", "fc_vf", "fc_abs_vs", "fc_we_lr", "fc_contact"]
...
def _plot_swarm(df, keys, path, key_x="geno", swarm=True, violin=False, width=2.6, suptitle="", col=None, palette=None, ylabel=None):
    ...
    i = 0
    fig, axes = plt.subplots(row, col, figsize=(col*width*SWARM_WIDTH_SCALE, row*width*SWARM_HEIGHT_SCALE), dpi=300) #,sharex=True), 
            ...
            for i in range(0, 1):#l):
                for j in range(i, l)[::-1]:
                    ...
                    if PLOT_ALL_PVAL or (0 < v < 0.05):
                        ...
                        ax.text(cur_x1, cur_y + 2*gap, get_pv_str(v), fontsize=FONT_SIZE*0.6, color=vc)
                        sig_i += 1
...
def plot_all_pair_out(all_geno_d, data_type, figure_name, out_file=None):
    ...
    plot_multi_info(multi_info, "", os.path.join(DATA_DIR, "multi-%s.png" % figure_name), col=3)
...
def ana_main(cmd=None, data_i=None):
    ...
    if cmd == "p":
        ...
        driver_out_d = {"/media/syf/SYF/T45": ["CSXTrp31 T45XTrp31 T45XShi31"],
                        "/media/syf/SYF/LC4": ["CSXTrp31 LC4XTrp31 LC4XShi31"], # 202107
                       	"/media/syf/SYF/LC9": ["CSXTrp31 LC9XTrp31 LC9XShi31"], # 202012
                        "/media/syf/SYF/LC13": ["CSXTrp31 LC13XTrp31 LC13XShi31"], # 201907
                        "/media/syf/SYF/LC17": ["CSXTrp31 LC17XTrp31 LC17XShi31"], # 201909
                        "/media/syf/SYF/LC21": ["CSXTrp31 LC21XTrp31 LC21XShi31"],
                        "/media/syf/SYF/LC25": ["CSXTrp31 LC25XTrp31 LC25XShi31"]} # 201911
        SWARM_PALATTE = [a for a in "krb"*7]
        PLOT_DRIVER_GROUP = 3
        ...
        mp_process_files(update_one_pair, get_all_pair_folder(VIDEO_DIR), POOL_SIZE)
