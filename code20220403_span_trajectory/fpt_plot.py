...
def plot_summary_by_name(names, dfs, filename, bouts=[], w_sc=1.2, h_sc=1, bottom=0.06, col=6, space=0.3, left=0.06, right=0.96, cb=None, n=1, save_pickle=False, save_svg=True, need_title=False, info_dir=None, sharex=False, count=None):
    fig_count = len(names)
    if not col:
        col = fig_count
    ...
    for idx, name in enumerate(names):
        ...
        fname_t = name.split("-")
        if name.startswith("polar") or name.startswith("partial_track_a"):
...
def plot_multi_info(fm, prefix="", out_path="multi.png", col=3):
    ...
    axes = axes.flatten()
    plt.subplots_adjust(left=0.02, right=1, top=0.95, bottom=0.05)
    ...
    for i in np.arange(0, 22):
        geno_fm = fm[:, i]
        for j, fi in enumerate(geno_fm):
            title = fi.split("-")[0]
            title = title.split("/")[-1]
            # bc = "w"
            color = ["r", "g", "b"]
            ax = axes[i]
            info = pickle.load(open(os.path.join(prefix, fi), 'rb'))
            df = info[-1]
            df[-3] = color[j]
            plot_line_by_info(ax, df, label=title)
            ax.legend()
            # info = load_fig_info(os.path.join(prefix, fi))[-1]
            # if info:
            #     try:
        ...
        #         if info[1] > 600:
        #             bc = "y"
        ax.set_title(title, fontsize=FONT_SIZE*2, backgroundcolor='w')
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
    plt.show()
    # plt.tight_layout()
